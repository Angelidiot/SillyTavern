...
### 阶段 123：PWA 离线导航首页兜底
- [x] 确认安装版 PWA 离线恢复带 query 导航可能 miss 当前 request cache
- [x] service worker 导航失败后先查当前 request，再回退缓存根页面
- [x] 保持 `/api/*` 和非 GET 请求不进入 CacheStorage
- [x] 补充 PWA contract 和浏览器 E2E 覆盖离线带 query 导航
- [x] 更新 README、设计文档和规划记录
- [x] 运行本地基础验证
- [x] 提交并推送到 GitHub fork
- [x] GitHub Actions 确认 Marketplace Wallet Checks 全链路通过
...
### 阶段 127：PWA 样式变更触发 Marketplace CI
- [x] 确认 Marketplace Wallet Checks path filters 漏监听 `public/style.css`
...
### 阶段 128：举报与拒绝理由前端长度校验
- [x] 确认 marketplace-wallet 对 reject/report 文本仍会静默截断
- [x] 改为超长本地提示并阻止请求，和后端长度契约一致
- [x] bump marketplace-wallet manifest 和 PWA 预缓存版本
- [x] 补充 UI contract 与浏览器 E2E
- [x] 运行本地基础验证
- [x] 提交并推送到 GitHub fork
- [x] GitHub Actions 确认 Marketplace Wallet Checks 全链路通过
- **状态：** complete

### 阶段 129：Library 安装忙碌态同步
- [x] 确认 `withBusyAsset()` 只重绘 Marketplace 列表，Library 按钮忙碌态不会同步
- [x] 在资产忙碌状态变化时同步重绘 My Library
- [x] bump marketplace-wallet manifest 和 PWA 预缓存版本
- [x] 补充 UI contract 和浏览器 E2E
- [x] 运行本地基础验证
- [x] 提交并推送到 GitHub fork
...
### 阶段 130：PWA 预缓存 filters 版本化
- [x] 确认 marketplace-wallet `filters.js` 仍以裸路径进入 PWA shell cache
- [x] 将 `filters.js` import 和 service worker 预缓存路径跟随扩展版本 query
- [x] bump marketplace-wallet manifest 和 PWA 预缓存版本
...
### 阶段 131：根 PWA manifest 语法门禁
- [x] 确认最快 syntax gate 只解析 marketplace-wallet manifest，未解析根 `public/manifest.json`
- [x] 将根 PWA manifest 加入 `scripts/check-marketplace-syntax.mjs` 静态 JSON 资产检查
...
### 阶段 137：Revise Cancel 退出编辑模式回归
- [x] 确认 Revise 后 Cancel 缺少浏览器回归覆盖
- [x] 新增 E2E 覆盖 Revise -> Cancel -> Save Draft 走 POST 而不是 PATCH
- [x] 修复 marketplace-wallet upload status hidden 被 display 样式覆盖
- [x] 更新 UI contract 锁定 hidden 状态样式
- [x] 运行本地基础验证
- [x] 提交并推送到 GitHub fork
- [x] GitHub Actions 确认 Marketplace Wallet Checks 全链路通过
- **状态：** complete
...
### 阶段 140：Admin Delist 成功闭环 E2E
- [x] 确认前端 Delist 只有 contract 字符串覆盖，缺少真实浏览器确认框/POST/刷新链路
- [x] 扩展 marketplace E2E mock，记录 `/api/market/assets/:id/delist` 调用并更新资产状态
...
### 阶段 141：Report Resolve Note 前端闭环
- [x] 确认后端和 API reference 支持 report resolution `note`，但前端 Resolve 仍发送空 body
- [x] 前端 Resolve 增加可选备注输入、本地 1000 字边界和 `{ note }` POST body
...
### 阶段 144：README runnable scripts 动态覆盖
- [x] 确认 README hosted marketplace/PWA 脚本覆盖测试仍使用硬编码清单
...
### 阶段 145：Report resolve 失败恢复 E2E
- [x] 确认 Report Queue 已覆盖加载失败 retry 和 resolve 成功路径，但缺少 resolve POST 失败恢复浏览器回归
...
### 阶段 150：钱包赠币 reason 边界
- [x] 确认 admin grant reason 会被 ledger 创建逻辑静默截断到 200 字符
- [x] 后端 grant 路由新增 200 字符显式校验，空白 reason 继续默认 `Admin grant`
...
- [x] marketplace E2E mock 新增 `failRejectOnceFor`，失败发生在资产状态/rejection_reason 变更前
- [x] 新增浏览器 E2E，断言第一次 reject 503 后队列仍保留资产、Reject 按钮恢复、creator submitted/rejected 计数不变
...
- [x] marketplace E2E mock 新增 `failDelistOnceFor`，失败发生在资产状态/delisted_at 变更前
