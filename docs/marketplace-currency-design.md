...
```text
draft -> submitted -> listed
                    -> rejected
listed -> delisted
listed -> suspended
```

历史 `approved` 状态只作为兼容输入处理；当前审核通过会直接进入 `listed/public`，未 `listed` 的资产不会进入普通用户公开浏览、详情、举报或购买入口。
...
当前本地 MVP 不做 multipart 服务端文件上传解析；内置 marketplace-wallet 扩展可读取本地 JSON 文件或粘贴的 JSON，把内容写入 `normalized_payload`，并按 payload 形状自动选择角色卡或世界书类型。上传表单会提交最多 20 个、每个 40 字符以内的 `tags`，以及后端同样支持的 `language` 和 `content_rating` 元数据，资产卡片、详情和搜索也会使用这些展示字段。后端会把 `metadata` 限制在 65536 字节以内、`normalized_payload` 限制在 1048576 字节以内，避免单个超大资产拖慢本地 JSON store、审核详情和移动端 PWA。正式 SaaS 再补角色卡 PNG/JSON、世界书 JSON、预设 JSON 的解析、对象存储和安全扫描流水线。
...
`npm run test:marketplace:smoke` 会先运行 demo seed 脚本，再临时启动真实 server，覆盖 creator 上传、提交、审批、Creator Center 统计、举报创建/队列/resolve、免费领取/安装和固定价购买的 admin grant、买家扣款、创作者收益、响应隐私 shape、Library 和文件落盘。
...
Review Queue 展示资产类型、创作者、价格、更新时间、标签和安全摘要片段；payload 仍只在管理员点击 Inspect 后通过资产详情权限懒加载。上传和修订入口会限制 `metadata`/`normalized_payload` 的 JSON 字节大小，submit 和 approve 也会复查该边界，防止旧数据或手工写入的 store 绕过限制。
...
`npm run test:pwa:e2e` 会用临时 server 和真实浏览器验证 `/login.html` 注册 `/service-worker.js`、`sillytavern-shell-v2` 缓存包含静态 shell 与 marketplace-wallet 扩展资源，并确认 `/api/health` 不会进入 CacheStorage。
