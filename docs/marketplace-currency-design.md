...
marketplace-wallet 钱包面板展示当前用户的最近流水，包含运营赠币、购买扣款、创作者收益等正负金额记录；完整账本仍以 `GET /api/wallet/ledger` 为准。管理员赠币 reason 必须是字符串，空值默认 `Admin grant`，超过 200 字符或非字符串值会被拒绝而不是静默截断。
