...
- Creator upload flow for draft assets, language/content rating metadata, bounded metadata/tags/payload size, payload shape validation, file/pasted JSON type auto-detect, and submit-for-review.
...
- `npm run test:hosted:docker` verifies the deployable Docker image exposes the hosted health endpoint and PWA shell; local machines without Docker should rely on CI for this check.
