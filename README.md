...
- Retryable side-panel error states for Creator Center, My Library, Wallet Activity, and Report Queue so transient API failures do not look like empty data.
...
- Admin coin grants for `bonus`, `paid`, and `earnings`, with `handle`, `userHandle`, or `targetHandle` recipient aliases and a 200-character reason limit.
...
Open `http://127.0.0.1:8000` on the same computer and click the top-right `市场 / 钱包` launcher, or open the Extensions panel and find `Marketplace & Wallet`. Users can view recent wallet ledger activity from the wallet panel.
...
On a physical phone, `127.0.0.1` points at the phone itself. Start the server with `npm start -- --listen=true`, then open `http://<your-computer-lan-ip>:8000` from the phone while both devices are on the same network, or use an HTTPS tunnel/hosted URL. Browser PWA install prompts and service workers require a secure context: HTTPS is safest for hosted/mobile testing, while localhost is only treated as secure on the same device. When the browser exposes `beforeinstallprompt`, SillyTavern shows an in-app `Install` action that calls the native PWA prompt and disappears after install or dismissal. The PWA service worker skips waiting, claims current clients after old cache cleanup, uses network-first navigation, falls back to the cached root shell for offline restored/query navigations, caches static shell assets, and never caches `/api/*` wallet, market, or chat requests.
