...
const routeNotes = new Map([
    ...
    ['POST /api/market/assets', 'Creates a draft character_card or world_book asset; body must be a JSON object with object metadata and normalized_payload, bounded text metadata/tags, supported price_type free/fixed_price, and positive safe integer price_coins for fixed_price assets.'],
    ['PATCH /api/market/assets/:id', 'Creator-only revision for draft or rejected assets; uses the same asset body validation as create and resets the asset to private draft before resubmission.'],
